...
function ImageCarousel({ images, title }) {
  ...
  // Auto-advance only when hovered
  useEffect(() => {
  ...
  return (
    <div
      className="relative h-44 overflow-hidden bg-slate-100 group"
    ...
    >
      ...
      >
        {images.map((url, i) => (
          <div key={i} className="flex-shrink-0 h-full" style={{ width: `${100 / total}%` }}>
            <img
              src={url}
              alt={`${title} photo ${i + 1}`}
      ...
      {/* Prev / Next — only show if >1 image */}
      {total > 1 && (
        <>
          <button
            onClick={prev}
            className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 backdrop-blur-sm z-10"
          >
            <MdChevronLeft size={18} />
          </button>
          <button
            onClick={next}
            className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/40 hover:bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 backdrop-blur-sm z-10"
          >
          ...
          {/* Counter badge */}
          <span className="absolute top-2.5 right-2.5 bg-black/50 text-white text-[10px] font-bold px-2 py-0.5 rounded-full backdrop-blur-sm z-10">
            {current + 1}/{total}
          </span>
        </>
      )}
...
/* ─── Property Card ──────────────────────────────────────────────── */
function PropertyCard({ post, onEdit, onDelete, onStatusChange, onDetails, statusUpdating }) {
  const [menuOpen, setMenuOpen] = useState(false);

  // Normalise field names — API returns camelCase, some fields differ from dummy
...
export default function MyProperties() {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null);  // post object
  const [deleteLoading, setDeleteLoading] = useState(false);

  // For status update feedback
  const [statusUpdating, setStatusUpdating] = useState(null); // post id
    ...
    >
      ...
      <div className="flex items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-extrabold text-slate-800 leading-tight">My Properties</h2>
          <p className="text-xs text-slate-400 mt-0.5">
            {loading ? "Loading…" : `${posts.length} listing${posts.length !== 1 ? "s" : ""} total`}
          </p>
        </div>
        <button
          onClick={() => navigate("/dashboard/addProperty")}
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white border border-slate-100 rounded-2xl gap-3">
            ...
            <div className="text-center">
              <p className="text-sm font-bold text-slate-600">
                {filter === "all" ? "No properties yet" : `No ${STATUS_CONFIG[filter]?.label ?? filter} properties`}
              </p>

...
const NAV_ITEMS = [
  ...
  { id: "myProperties",  label: "Properties",        path: "/dashboard/myProperties",  Icon: MdOutlineHome },
  ...
  { id: "messages",      label: "Messages",          path: "/dashboard/messages",      Icon: MdOutlineMessage, badgeKey: "messages" },
  { id: "users",         label: "All Users",         path: "/dashboard/users",         Icon: MdOutlinePersonOutline, adminOnly: true },
  { id: "queries",       label: "Queries Messages",  path: "/dashboard/queries",       Icon: MdInbox, badgeKey: "queries", adminOnly: true },
];
...
export default function DashSidebar({ badges = {} }) {
  ...
  // Merge real counts into badges
  const resolvedBadges = { ...badges, messages: totalUnread };
  ...
  return (
    <div
      ...
      style={{ top: NAVBAR_H, left: 0, width: W_STRIP, height: `calc(100vh - ${NAVBAR_H}px)`, zIndex: 40 }}
    >
      <nav className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide px-2 py-3">
        {visibleItems.map(({ id, label, path, Icon, badgeKey }) => {
          const count       = badgeKey ? (resolvedBadges[badgeKey] ?? 0) : 0;
            ...
            <NavLink
              ...
              className={({ isActive }) => {
                ...
                return [
                  ...
                  active ? "bg-gradient-to-r from-gray-50 to-purple-50" : "hover:bg-slate-50",
                ].join(" ");
              }}
            >
              {({ isActive }) => {
                ...
                return (
                  <>
                    ...
                    <span className="relative">
                      <Icon size={20} className={active ? "text-gray-700" : "text-slate-400"} />
                      {count > 0 && (

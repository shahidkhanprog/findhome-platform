...
export const createPost = async (req, res) => {
    ...
    const {
        title, price, images, address, city,
        bedroom, bathroom, latitude, longitude,
        listingType, property, status,
        // postDetails fields (optional — can be added in same request)
        desc, utilities, pet, income, size,
...
export const createPostDetails = async (req, res) => {
    ...
    try {
        ...
        if (!canModify(tokenUserId, isAdmin, post)) {
            return res.status(403).json({ message: "Forbidden: You can only add details to your own posts" });
        }
...
export const updatePost = async (req, res) => {
    ...
    try {
        ...
        const {
            ...
            listingType, property, status,
            // postDetails fields
            desc, utilities, pet, income, size, school, bus, restaurant,
        } = req.body;
        ...
        const postData = {
            ...
            ...(longitude   !== undefined && { longitude:   longitude.toString().trim()  }),
        ...
        const hasDetailsUpdate = [desc, utilities, pet, income, size, school, bus, restaurant]
            .some((v) => v !== undefined);
                    ...
                    postDetails: {
                        upsert: {
                            ...
                            update: {
                                ...
                                ...(restaurant !== undefined && { restaurant: parseInt(restaurant, 10)  }),
                            },
                        },
                    },
                }),
...
export const getUserPosts = async (req, res) => {
    ...
    if (tokenUserId !== targetUserId && !isAdmin) {
        return res.status(403).json({ message: "Forbidden: You can only view your own posts" });
    }
